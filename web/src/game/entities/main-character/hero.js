...
class Hero extends Actor {
  ...
  constructor(game, sprite) {
    ...
    const charStats = Globals.CHARACTERS[charKey] || Globals.CHARACTERS.brian;

    this.charStats = charStats;
    this.resetHealth(charStats.hp);

    // Apply character tint
    if (charStats.tint !== 0xffffff) {
      sprite.tint = charStats.tint;
    }
  ...
  _setupBody() {
    ...
    this.weight = this.charStats.weight;
  ...
  _attachAnimEvents() {
    this.anims.punch.onComplete.add(() => {
      ...
      this.state.damage = this.charStats.punchDmg;
    ...
    this.anims.kick.onComplete.add(() => {
      ...
      this.state.damage = this.charStats.kickDmg;
  ...
  update(enemies) {
    ...
    if (!this.state.isAttacking) {
      ...
      if (this.controls.up && !this.controls.down) {
        ...
        this._sprite.body.velocity.x =
          this.controls.left || this.controls.right ?
            this._sprite.body.velocity.x : 0;
        this._sprite.body.velocity.y = -this.charStats.speed;
      }
      ...
      if (this.controls.left && !this.controls.right) {
        ...
        this._sprite.body.velocity.x = -this.charStats.speed;
